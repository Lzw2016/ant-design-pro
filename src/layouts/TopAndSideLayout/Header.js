...
class HeaderView extends PureComponent {
  ...
  render() {
    const { handleMenuCollapse, selectedKeys, handleFirstLevelMenuClick, setting } = this.props;
    const { navTheme, fixedHeader } = setting;
    const width = this.getHeadWidth();
    return (
      <Layout.Header style={{ padding: 0, width }} className={fixedHeader ? styles.fixedHeader : ''}>
        <TopNavHeader
          theme={navTheme}
          Authorized={Authorized}
          onCollapse={handleMenuCollapse}
          selectedKeys={selectedKeys}
          handleFirstLevelMenuClick={handleFirstLevelMenuClick}
